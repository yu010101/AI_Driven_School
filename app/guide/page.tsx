...
const guideCategories = [
  ...
  },
  {
    id: 'claude-code',
    title: 'Claude Code攻略',
    description: 'Anthropic公式AIエディタを完全攻略',
    color: '#0A0A0A',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {guideList.map((guide, index) => (
              ...
              >
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 bg-[#0A0A0A] rounded-2xl flex items-center justify-center text-white font-bold text-xl group-hover:scale-110 transition-transform">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap gap-2 mb-3">
                    {guide.badges?.map((badge) => (
                      <span
                        key={badge}
                        className="px-2 py-0.5 text-xs font-medium rounded-full bg-[#0A0A0A]/10 text-[#0A0A0A]"
                      >
                        {badge}
                      </span>
                    ))}
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold mb-2 group-hover:text-[#0A0A0A] transition-colors">
                    {guide.title}
                  </h3>
                ...
                <div className="flex-shrink-0 self-center">
                  <span className="inline-flex items-center gap-2 text-[#0A0A0A] font-semibold group-hover:gap-3 transition-all">
                    読む
